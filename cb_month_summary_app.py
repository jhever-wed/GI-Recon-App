...
with tab2:
    ...
    st.header("🚫 Mismatch Details")
    st.dataframe(mis)
    # Summary by DATE & ACCOUNT
    # Re-merge original by-date-account for mismatch details
    details = pd.merge(
        df1.groupby(["CB", "DATE", "ACCOUNT"], dropna=False)[["QTY", "FEE"]]
           .sum().reset_index().rename(columns={"QTY":"QTY_ATLANTIS","FEE":"FEE_ATLANTIS"}),
        df2.groupby(["CB", "DATE", "ACCOUNT"], dropna=False)[["QTY", "FEE"]]
           .sum().reset_index().rename(columns={"QTY":"QTY_GMI","FEE":"FEE_GMI"}),
        on=["CB", "DATE", "ACCOUNT"], how="outer"
    ).fillna(0)
    ...
    summary_date_acc = mis_details.groupby(["DATE", "ACCOUNT"], dropna=False)[[
        "QTY_ATLANTIS","FEE_ATLANTIS","QTY_GMI","FEE_GMI","QTY_DIFF","FEE_DIFF"
    ]].sum().reset_index()
    st.header("📋 Mismatch Summary by Date & Account")
    st.dataframe(summary_date_acc)

# Download full report with CB summary and date-account summary
st.subheader("📥 Download Full Report")
buf = io.BytesIO()
with pd.ExcelWriter(buf, engine="openpyxl") as writer:
    merged.to_excel(writer, sheet_name="CB_Summary", index=False)
    summary_date_acc.to_excel(writer, sheet_name="Mismatch_Summary", index=False)
buf.seek(0)
